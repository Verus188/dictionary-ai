  ...
  const response = (
    await AIController.generateAIText(
      getStoryInitializationPrompt(
        ...
        ctx.get(storySettingsAtoms.chunkLength),
        ctx.get(storySettingsAtoms.educationLanguage),
        ctx.get(storySettingsAtoms.storyLanguageDifficulty),
        ctx.get(dictionaryCardsAtom),
        ctx.get(storySettingsAtoms.storyPrompt),
 ...
 */
export const generateStoryChunksAction = reatomAsync(
  async (ctx): Promise<StoryChunkVariants> => {
 ...
 */
export const generateStoryActionsAction = reatomAsync(
  async (ctx): Promise<ChunkActions> => {
